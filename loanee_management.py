...
def record_new_borrowing():
    ...
    cur.execute("""
        SELECT loanee_id, name, age, gender, employment_status, income, credit_score, address, city, 
               state, zip_code, country, email, phone_number, marital_status, dependents, education_level, 
               employer, job_title, years_employed
        FROM loanee 
        WHERE loanee_id = %s;
    """, (loanee_id,))
    ...
    (loanee_id, name, age, gender, employment_status, income, credit_score, address, city, 
     state, zip_code, country, email, phone_number, marital_status, dependents, education_level, 
     employer, job_title, years_employed) = loanee_data
    ...
    # Update the existing borrowing record in the database
    update_query = """
    UPDATE loanee 
    SET amount_borrowed = %s, date_borrowed = %s, expected_repayment_date = %s, amount_to_be_repaid = %s, loan_purpose = %s, 
        loan_type = %s, interest_rate = %s, loan_term = %s
    WHERE loanee_id = %s;
    """

    data = (amount_borrowed, date_borrowed, expected_repayment_date, amount_to_be_repaid, loan_purpose, loan_type, 
            interest_rate, loan_term, loanee_id)
